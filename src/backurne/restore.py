...
from .disk import deactivate_vg
from .disk import filter_children
from .disk import get_fs_info
from .disk import get_mapped
from .disk import get_next_nbd
from .disk import resolv_qemu_nbd
from .disk import resolv_vmfs
from .disk import wait_dev
...
class Restore:
    ...
    def mount_dev(self, dev, ignore_mapped=False):
        ...
        if info["parttype"] == "0x42":
            ...
            return False
        if (
            info["fstype"] is not None
            and info["mountpoint"] is None
            and info["fstype"] != "LVM2_member"
        ):
    ...
    def umount_tree(self, tree, first_pass=False):
        ...
        if tree.name.image is not None and first_pass is False:
            ...
            Log.debug(
                f"\t{tree.name.dev}: rbd unprotect --snap {tree.name.parent_snap} {tree.name.parent_image}"
            )
