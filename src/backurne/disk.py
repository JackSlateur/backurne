...
def get_fs_info(dev):
    if dev is None:
        return []
    info = lsblk(
        "-I", 8, "-p", "-o", "+NAME,FSTYPE,SIZE,MOUNTPOINT,PARTTYPE", "-J", dev
    )
...
def add_part(part, parent, extended, qemu_nbd=None):
    if part["fstype"] == "LVM2_member":
        ...
        vmdks = "{}/*/*-flat.vmdk".format(part["mountpoint"])
        for vmdk in glob.glob(vmdks):
            ...
            if len(qcow2) == 0:
                sub = Node(
                    Part(dev=vmdk, size=vmdk_size), parent=node, qemu_nbd=qemu_nbd
                )
                continue
            qcow2 = qcow2[0]
            nbd = resolv_qemu_nbd(qcow2)
            sub = Node(
                Part(dev=qcow2, size=vmdk_size, qemu_nbd=nbd),
                parent=node,
                qemu_nbd=qemu_nbd,
            )
...
def print_node(pre, _node):
    ...
    if node.parent_image is not None:
        Log.info("{}rbd {} / snap {}".format(pre, node.parent_image, node.parent_snap))
        return

    if node.mountpoint is not None:
        msg = "on {} ".format(node.mountpoint)
    else:
        msg = ""
    ...
    if node.dev.endswith(".vmdk"):
        ...
        dev = node.dev
        fstype = "fstype {}".format(node.fstype)
    if has_debug(Log):
        Log.info(
            "%s%s %s(%s, size %s, nbd %s, vmfs %s, mnt %s, real dev %s, ldm %s)"
            % (
                pre,
                dev,
                msg,
        ...
        Log.info("{}{} {}({}, size {})".format(pre, dev, msg, fstype, node.size))
